...
import pygame
from sequtus.libs import vectors, geometry, drawing
...
def strip_actor(the_actor):
    ...
    def _strip_order(order):
        ...
        if target != None:
            if type(target) not in (int, str, unicode, list, tuple):
                target = target.oid
...
def build_template_cache(template, engine, sim):
    ...
    for a in template['abilities']:
        ability_type = sim.ability_types[a]
        ability_damage = {}
